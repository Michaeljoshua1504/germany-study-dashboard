...
### Admission > Reference Notes Tab
- New sub-tab under Admission (4th, after Checklist & Contacts): "📖 Reference Notes"
- **Section id:** `tab-admission-reference` — same auth-gated placeholder pattern, content in Supabase `page_sections`, `section_key = 'admission-reference'`
- **Contents:** accordion (`<details>/<summary>`, custom `.ref-topic` class, own scoped CSS) of non-checklist background info from the Access Data letter + 3 orientation PDFs: login portals table, account basics, MFA setup recap, university fast facts, communication culture rules, consultation hours, CampusCard coverage, sports facilities, bank/residence-permit document requirements, health insurance notes, scam awareness.
- **SECURITY — deliberately excluded:** actual password and online registration number are NOT stored anywhere in this tab or repo. Repo is public (confirmed via API), so anything pushed to `supabase/update.sql` sits in plaintext git history even though the live Supabase table itself is RLS-protected. Only non-secret account basics (username, university email, Matrikelnummer) are included, with an inline note-box explaining the omission. Apply the same caution to any future content request involving live credentials.
- **To update:** same pattern — rebuild the accordion HTML (`topic()`/`row()` helper functions), push via `INSERT ... ON CONFLICT (section_key)` to `supabase/update.sql` with `section_key = 'admission-reference'`, dollar-quoted as `$ADMREF$...$ADMREF$`.
